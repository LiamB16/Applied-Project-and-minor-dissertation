...
label2 = tk.Label(my_w,  image = bkimg, 
        bg = "white", fg = "black")
...
def display():
    c.execute("select crime from criminal_record where ID = 'G0037748';")
    i=3
    for student in c: 
...
def del_data(s_id): # delete record 
    try:
        ...
        if my_var:
            ...
            select_query = 'insert into daily_activity values ("G00377746", "Deleted crime", curdate(), curtime());'
